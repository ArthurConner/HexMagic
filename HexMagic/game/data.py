...
# %% auto #0
__all__ = ['PIECE_DEFAULTS', 'Resources', 'PieceType', 'Instruction', 'Piece', 'Settlement', 'WatershedFlow', 'TradeRoute',
           'Kingdom', 'GameBoard', 'ActiveGame', 'PieceRecord', 'SettlementRecord', 'KingdomRecord', 'KingdomHex',
           'GameStorage']
...
# %% ../../nbs/game/02_data.ipynb #04bd1351
class PieceType(Enum):
    """Chess piece types with default stat profiles."""
    KING   = "king"
    QUEEN  = "queen"
    BISHOP = "bishop"
    KNIGHT = "knight"
...
class Piece:
    ...
    rules: List[int] = field(default_factory=list)  # list of Instruction values
    cursor: int = 0             # position in rules list
    patrol: bool = True         # loop rules when cursor hits end?
    ...
    def encode(self) -> str:
        ...
        for f in self.PIECE_FIELDS:
            ...
            elif isinstance(v, list):  vals.append(','.join(str(x) for x in v))
            elif isinstance(v, bool):  vals.append('1' if v else '0')
            elif isinstance(v, Enum):  vals.append(v.value)
    ...
    def decode(cls, line: str) -> 'Piece':
        ...
        # Sensible defaults for diet/capacity if zero
        ...
        # Rules list
        rules_str = kw.get('rules', '')
        kw['rules'] = [int(x) for x in rules_str.split(',') if x] if rules_str else []
...
def save(self: Piece, db: GameStorage = None, world_id: int = 0,
    ...
    record = PieceRecord(
        ...
        cursor=self.cursor,
        patrol=1 if self.patrol else 0,

...
# %% auto 0
__all__ = ['Watershed', 'DrainageBasins', 'find_local_minima']
...
# %% ../nbs/08_hydrology.ipynb 42
@patch
def trace_to_terminal(self: Terrain, start: int) -> list[int]:
    """Trace downhill from start until ocean or local minimum."""
    path = [start]
    current = start
    visited = {start}
    ...
    while True:
        ...
        if lowest is None or lowest in visited:
            break  # Local minimum (lake)
        if self.elevations[lowest] <= 0:
            path.append(lowest)  # Include ocean hex
            break
...
# %% ../nbs/08_hydrology.ipynb 52
@patch
def quickTerrain(self: TerraDemo, bounds=None, radius=15, num_plates=16, ocean_fraction=0.6):
    """Generate terrain without climate overhead."""
    if bounds is None:
        bounds = MapRect(MapCord(0, 0), MapSize(800, 800))
    
    terrain, plates = TerrainFactory.create_ocean_world(
        bounds=bounds, preset='monsoon', radius=radius,
...
@patch
def countLakes(self: TerraDemo, ittr=10):
    """Count local minima across multiple terrain generations."""
    data = np.zeros(ittr)
    
    for i in range(len(data)):
        terrain = self.quickTerrain()
        data[i] = len(find_local_minima(terrain))
...
@patch
def carve_to_ocean(self: Terrain, num_lakes: int = 5, max_iters: int = 10) -> 'Terrain':
    ...
    for iteration in range(max_iters):
        ...
        print(f"Iter {iteration}: {len(minima)} → {new_count} lakes")

...
# %% auto #0
__all__ = ['TerrainTemplate', 'WeatherColor', 'WeatherFetcher']
...
# %% ../nbs/06_weather.ipynb #898120f2
@patch
def _clean_junk(self:TerraDemo,terrain):
    """ there were a bunch of bad fields in these demo files"""
    for field in ["rainfall", "tempLow", "tempHigh"]:
        if field in terrain.fields:
            del terrain.fields[field]
    return terrain
...
def bayArea_map(self:TerraDemo):
    ...
    with resources.files('HexMagic').joinpath('data/templates/BayArea.txt').open() as f:
        return self._clean_junk(Terrain.decode(f.read()))


# %% ../nbs/06_weather.ipynb #4fd2c6a5
@patch
def marin_map(self:TerraDemo):
    """Load Marin terrain with proper geographic bounds."""
    with resources.files('HexMagic').joinpath('data/templates/marin.txt').open() as f:
        terrain = self._clean_junk(Terrain.decode(f.read()))
    terrain.geo  = GeoBounds(
    lat_min=37.83,   # Golden Gate / south Marin
    lat_max=38.24,   # Tomales Point (north end of Pt Reyes)
    lon_min=-123.05,  # west of Point Reyes headlands
    lon_max=-122.45   # east Marin / Richardson Bay
    )
    terrain.climate  = ClimatePreset(
    name='California',
    lat_range=(37.83, 38.24),  # Your actual CA bounds
...
# %% ../nbs/06_weather.ipynb #b99552b0
@patch
def hundred_years_map(self: TerraDemo):
    """Load Hundred Years War terrain (England/France)."""
    with resources.files('HexMagic').joinpath('data/templates/hundred_years_80x70.txt').open() as f:
        terrain = Terrain.decode(f.read())
    
    terrain.geo = GeoBounds(
        lat_min=43.0,   # Southern France
        lat_max=54.0,   # Northern England
        lon_min=-5.0,   # Brittany/Cornwall
        lon_max=5.0     # Low Countries
    )
    terrain._compute_hex_coordinates()
    
    terrain.climate = ClimatePreset(
        name='Maritime Temperate NW Europe',
        lat_range=(43, 54),
        base_temp_range=(12, 10),
        wind_speed=10.0,
        wind_dir=250.0,  # Prevailing westerlies from Atlantic
        precip_base=0.08,
        nm=0.010,
        hw=1800.0,
        cw=0.003,
        ...
        fall_time=1500.0,
        lapse_rate=6.0,
        precip_bins=(500, 800, 1100, 1500),
        description="Maritime temperate. Mild wet winters, cool summers. Atlantic weather systems."
    )
    self._clean_junk(terrain)
    return terrain

@patch
def thirteen_colonies_map(self: TerraDemo):
    """Load Thirteen Colonies terrain (US East Coast)."""
    with resources.files('HexMagic').joinpath('data/templates/thirteen_colonies.txt').open() as f:
        terrain = Terrain.decode(f.read())
    
    terrain.geo = GeoBounds(
        lat_min=32.0,   # Georgia
        lat_max=45.0,   # Maine/NH border
        lon_min=-82.0,  # Interior
        lon_max=-69.0   # Atlantic coast
    )
    terrain._compute_hex_coordinates()
    
    terrain.climate = ClimatePreset(
        name='Humid Subtropical/Continental',
        lat_range=(32, 45),
        base_temp_range=(16, 12),  # Warm south, cool north
        wind_speed=8.0,
        wind_dir=270.0,  # Westerlies, but hurricanes from SE
        precip_base=0.10,
        nm=0.008,
        hw=2200.0,
        cw=0.004,
        ...
        fall_time=1000.0,
        lapse_rate=6.0,
        precip_bins=(900, 1200, 1500, 1900),
        description="Humid subtropical to continental. Hot humid summers, variable winters. Appalachian orographic effects."
    )
    self._clean_junk(terrain)
    return terrain

@patch
def japan_korea_map(self: TerraDemo):
    """Load Japan/Korea terrain."""
    with resources.files('HexMagic').joinpath('data/templates/japan_korea.txt').open() as f:
        terrain = Terrain.decode(f.read())
    
    terrain.geo = GeoBounds(
        lat_min=33.0,   # Kyushu/southern Korea
        lat_max=45.0,   # Hokkaido
        lon_min=125.0,  # Korean peninsula
        lon_max=146.0   # Eastern Japan
    )
    terrain._compute_hex_coordinates()
    ...
    terrain.climate = ClimatePreset(
        ...
        lat_range=(33, 45),
        base_temp_range=(15, 12),
        wind_speed=9.0,
        wind_dir=135.0,  # SE monsoon in summer
        precip_base=0.12,
        nm=0.009,
        hw=2400.0,
        cw=0.005,  # Strong orographic - mountainous terrain
        conv_time=900.0,
        fall_time=900.0,
        lapse_rate=5.8,
        precip_bins=(1000, 1500, 2000, 2800),
        description="Humid continental to subtropical. Monsoon summers, cold winters. Sea of Japan snow effect."
    )
    self._clean_junk(terrain)
    return terrain

@patch
def mongol_europe_map(self: TerraDemo):
    """Load Mongol Empire to Europe terrain."""
    with resources.files('HexMagic').joinpath('data/templates/mongol_europe_70x80.txt').open() as f:
        terrain = Terrain.decode(f.read())
    
    terrain.geo = GeoBounds(
        lat_min=35.0,   # Persia/Middle East
        lat_max=55.0,   # Russian steppe
        lon_min=20.0,   # Eastern Europe
        lon_max=120.0   # Mongolia
    )
    terrain._compute_hex_coordinates()
    
    terrain.climate = ClimatePreset(
        name='Continental Steppe',
        lat_range=(35, 55),
        base_temp_range=(10, 6),  # Extreme continental range
        wind_speed=12.0,
        wind_dir=270.0,  # Westerlies across the steppe
        precip_base=0.04,  # Dry steppe/desert interior
        nm=0.015,
        hw=1400.0,
        cw=0.002,  # Few mountains to create orographic rain
        conv_time=2000.0,
        fall_time=2000.0,
        lapse_rate=7.0,
        precip_bins=(200, 400, 600, 900),
        description="Continental steppe. Extreme temperature range. Very dry interior, distance from oceans."
    )
    self._clean_junk(terrain)
    return terrain


# %% ../nbs/06_weather.ipynb #4d885c45
class TerrainTemplate:
    _demo = TerraDemo()
    maps = {name.removesuffix('_map'): name 
        for name in dir(_demo) if name.endswith('_map') and not name.startswith('_')}
    
    def __getattr__(self, key):
        if key in self.maps:
            return getattr(self._demo, self.maps[key])
        raise AttributeError(f"No template '{key}'. Available: {list(self.maps)}")
...
# %% ../nbs/06_weather.ipynb #a0bcf8ea
@dataclass
class WeatherFetcher:
    """Fetch 30-year climate normals from Open-Meteo for biome classification."""
    
    ARCHIVE_URL = "https://archive-api.open-meteo.com/v1/archive"
    
    # Climate normal period (WMO standard)
    start_date: str = "1991-01-01"
    end_date: str = "2020-12-31"
    
    async def fetch_point_climate(self, client: httpx.AsyncClient, 
                                   lat: float, lon: float) -> dict | None:
        """Fetch climate normals for a single point."""
        try:
            resp = await client.get(self.ARCHIVE_URL, params={
                ...
                "daily": ",".join([
                    ...
                    "temperature_2m_min",
                    "precipitation_sum",
                    "et0_fao_evapotranspiration",  # Reference ET (Penman-Monteith)
                ]),
                "timezone": "auto"
            }, timeout=30)
            ...
            annual_precip = np.sum(precip) / 30  # 30 years -> annual mean
            annual_et0 = np.sum(et0) / 30 if et0 else None
            
            # Aridity index (UNEP definition: P/PET)
            aridity_index = (annual_precip / annual_et0) if annual_et0 and annual_et0 > 0 else None
            
            # Temperature seasonality (standard deviation of monthly means)
            if len(temps) >= 365:
                # Group into ~monthly chunks (30 days)
                monthly_temps = [np.mean(temps[i:i+30]) for i in range(0, len(temps)-30, 30)]
                temp_seasonality = np.std(monthly_temps) if monthly_temps else 0
            else:
                temp_seasonality = 0
            
            # Precipitation seasonality (coefficient of variation of monthly totals)
            if len(precip) >= 365:
                monthly_precip = [np.sum(precip[i:i+30]) for i in range(0, len(precip)-30, 30)]
                precip_mean = np.mean(monthly_precip)
                precip_seasonality = (np.std(monthly_precip) / precip_mean * 100) if precip_mean > 0 else 0
            else:
                precip_seasonality = 0
            
            # Estimate humidity from temperature range (approximation)
            # Lower diurnal range = higher humidity
            if temps_max and temps_min and len(temps_max) == len(temps_min):
                diurnal_range = np.mean([mx - mn for mx, mn in zip(temps_max, temps_min) if mx and mn])
                # Rough estimate: DR of 5°C ≈ 80% RH, DR of 20°C ≈ 30% RH
                estimated_humidity = max(20, min(95, 100 - diurnal_range * 3.5))
            else:
                estimated_humidity = 50  # fallback
            
            return {
                'annual_temp': annual_temp,
                'annual_precip': annual_precip,
                'annual_pet': annual_et0,
            ...
            print(f"Error fetching climate ({lat}, {lon}): {e}")
            return None
    
    async def fetch_batch(self, points: list[tuple[float, float]], 
                          max_concurrent: int = 5) -> list[dict | None]:
        """Fetch climate for multiple points with rate limiting.
        
        Note: Open-Meteo is generous but 30 years of daily data is heavy.
        ...
        async def bounded_fetch(client, lat, lon):
            async with semaphore:
                result = await self.fetch_point_climate(client, lat, lon)
    ...
    async def populate_terrain(self, terrain: Terrain, 
                                progress_callback=None) -> Terrain:
        """Add climate normal fields to an existing terrain.
        
        Requires terrain to have 'latitude' and 'longitude' fields.
        """
        if 'latitude' not in terrain.fields or 'longitude' not in terrain.fields:
            raise ValueError("Terrain must have latitude/longitude fields")
        
        lats = terrain.fields['latitude']
        lons = terrain.fields['longitude']
        points = list(zip(lats, lons))
        
        n_hexes = len(points)
        print(f"Fetching climate normals for {n_hexes} hexes...")
        ...
        batch_size = 50
        for i in range(0, n_hexes, batch_size):
            batch_points = points[i:i+batch_size]
            results = await self.fetch_batch(batch_points)
            
            for j, result in enumerate(results):
                idx = i + j
                if result:
                    terrain.fields['temperature'][idx] = result['annual_temp']
                    terrain.fields['precipitation'][idx] = result['annual_precip']
                    terrain.fields['climate_pet'][idx] = result['annual_pet'] or 0
                    terrain.fields['aridity_index'][idx] = result['aridity_index'] or 0
                    terrain.fields['temp_seasonality'][idx] = result['temp_seasonality']
                    terrain.fields['precip_seasonality'][idx] = result['precip_seasonality']
                    terrain.fields['humidity'][idx] = result['estimated_humidity']
            
            if progress_callback:
                progress_callback(min(i + batch_size, n_hexes), n_hexes)
            else:
                print(f"  {min(i + batch_size, n_hexes)}/{n_hexes} hexes...")

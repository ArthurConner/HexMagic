...
class Plate:
    def __init__(self,region:HexRegion,symbol="♠️",direction=4):
        ...
        self.id = ""  # Root plates get "1", "2", etc.
    ...
    def decode(cls, s: str, hex_grid: HexGrid) -> 'Plate':
        ...
        region = HexRegion(hexes, hexGrid=hex_grid)
...
def subdivide(self: Plate, depth: int = 2, min_size: int = 10, 
    ...
    sub_plates = []
    for child_index, region_positions in enumerate(regions.values()):
        ...
        if indices:
            ...
            sub_plate.id = f"{self.id}.{child_index}"
...
class TerrainGenerator:
    def __init__(self, 
        ...
        subdivisions=3,
        min_subplate_size=3,
        plate_base=6,  # Base for encoding plate IDs
        # Ocean configuration  
        ocean_fraction=0.4,
        oceanic_sides=[],
        edge_factor=1.5,
        ...
        self.min_subplate_size = min_subplate_size
        self.plate_base = plate_base
        self.ocean_fraction = ocean_fraction
        self.oceanic_sides = oceanic_sides
    ...
    def encode_plate_id(self, id_string):
        """Convert "1.2.5" -> integer using plate_base encoding."""
        if not id_string:
            return 0
        digits = [int(d) for d in id_string.split('.')]
        return sum(d * self.plate_base**i for i, d in enumerate(digits))
    
    def generate(self):
        """Generate terrain with plate tracking."""
        if self.seed is not None:
            np.random.seed(self.seed)
            random.seed(self.seed)
        
        params = self.age_params.get(self.age, self.age_params['middle'])
        
        # Create grid using create_cropped_grid
        style = StyleCSS("HexStyle", fill="lightgray", stroke="blue", stroke_width=1)
        grid = HexGrid.create_cropped_grid(self.hexDim, self.radius, style, self.ratio)
        
        # Create terrain and assign grid
        mySize = MapSize(grid.builder.width, grid.builder.height)
        myBounds = MapRect(MapCord(0, 0), mySize)
        terrain = Terrain(myBounds, radius=self.radius)
        terrain.assign_grid(grid)
        
        # Phase 1: Generate coarse plates for coastlines
        coarse_plates = Plate.voronoi(grid, self.num_plates)
        
        # Assign initial IDs
        for i, plate in enumerate(coarse_plates):
            plate.id = str(i)
        
        # Mark oceanic plates
        coarse_plates = mark_oceanic_plates(coarse_plates, ocean_fraction=self.ocean_fraction, seed=self.seed)
        if self.oceanic_sides:
            mark_oceanic_by_sides(coarse_plates, grid, factor=self.edge_factor, oceanic_sides=self.oceanic_sides)
        
        # Phase 2: Subdivide for detail (preserving plate kind)
        plates = []
        for plate in coarse_plates:
            sub = plate.subdivide(depth=self.subdivisions, min_size=self.min_subplate_size)
            plates.extend(sub)
        
        # Build plate map and populate plate_id field
        plate_map = {}
        for plate in plates:
            for idx in plate.hexes:
                plate_map[idx] = plate
        
        terrain.fields['plate_id'] = np.array([
            self.encode_plate_id(plate_map[i].id) if i in plate_map else 0
            for i in range(len(grid.hexes))
        ])
        ...
        # Generate elevations based on formation type
        if self.formation_type == 'ocean_distance':
            elevations = _elevation_ocean_distance(grid, plates, params, self.elevation_scale)
        elif self.formation_type == 'ridge':
            elevations = _elevation_ridge(grid, plates, boundaries, params, self.elevation_scale)
        elif self.formation_type == 'volcanic':
            elevations = _elevation_volcanic(grid, plates, params, self.elevation_scale)
        elif self.formation_type == 'rolling':
            elevations = _elevation_rolling(grid, plates, params, self.elevation_scale)
        elif self.formation_type == 'rift':
            elevations = _elevation_rift(grid, plates, boundaries, params, self.elevation_scale)
        elif self.formation_type == 'crater':
            elevations = _elevation_crater(grid, plates, params, self.elevation_scale)
        else:
            raise ValueError(f"Unknown formation_type: {self.formation_type}")
...
# %% ../nbs/04_voronoi.ipynb 73
@patch
def decode_plate_id(self:TerrainGenerator, encoded):
    if encoded == 0:
        return "0"
    digits = []
    while encoded > 0:
        digits.append(str(encoded % self.plate_base))
        encoded //= self.plate_base
    return ".".join(digits)

@patch
def hexes_from_plate(self:TerrainGenerator, terrain, root_id):
    """Get hex indices whose plate_id descends from root_id."""
    root_encoded = self.encode_plate_id(root_id)
    mask_mod = self.plate_base ** (root_id.count('.') + 1)
    return [i for i, pid in enumerate(terrain.fields['plate_id']) 
            if pid % mask_mod == root_encoded]
